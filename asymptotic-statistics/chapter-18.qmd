...
For a gentler process-level development, see [@sen2022, §§9.2--10.1]. For systematic treatments of weak convergence in metric spaces, see [@kosorok2008, chs. 6--7] and @vanDerVaartWellner1996. The measurable-map and image-measure viewpoint is developed in [@pollard2002, §§2.2 and 2.9].

:::

::: {.callout-note title="Coverage, omissions, and destinations"}

| Source material | Treatment here | Main destination |
|---|---|---|
| §18.1, Definition 18.1, Lemma 18.2, Examples 18.5, 18.7, and 18.8 | Core definitions and selected spaces | Random functions in [Chapter 19](chapter-19.qmd#donsker-classes) and Banach-valued maps in [§25.7](chapter-25.qmd#sec-efficiency-delta-banach) |
| §18.2, Lemma 18.9, and Theorem 18.10 | Exact statements; proofs linked to their genuinely identical Chapter 2 arguments | The process-level continuous mapping theorem and outer-probability conventions |
| Theorem 18.11 | Core; complete varying-map argument | The [functional delta method](chapter-20.qmd#theorem-20-8) and changing statistical maps |
| Theorem 18.12 | Exact statement, proof deferred as in the source | Tight subsequences used in Chapter 25 |
| §18.3, Theorem 18.14, and Lemma 18.15 | Core; complete layered proofs | Donsker classes, stochastic equicontinuity, and [Z-estimation](chapter-19.qmd#theorem-19.26-infinite-dimensional-z-estimation) |
| Examples 18.3, 18.4, and 18.6; Lemma 18.13 | Safely omitted or summarized by examples already used here | No additional prerequisite for the route to Chapter 25 |
...
When $\mathbb D_n=\mathbb D$ and $g_n=g_0$, the sequential condition is ordinary continuity on the support of $X$, so this result reduces to [Theorem 2.3](chapter-2.qmd#theorem-continuous-mapping). Allowing both the domain and the map to vary is the form used in the [functional delta method](chapter-20.qmd#theorem-20-8).
...
$$
F_n(t)=\frac1n\sum_{i=1}^n\mathbf1\{Z_i\leq t\},
\qquad t\in\mathbb R.
$$
...
::: {.callout-tip title="From random functions to Chapter 25"}
