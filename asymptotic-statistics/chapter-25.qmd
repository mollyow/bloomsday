...
::: {.callout-note title="Coverage and intentional compression" collapse="true"}

These notes cover §§25.1–25.12 and the numbered results and examples used on the route through the chapter. The three rows in **How the chapter fits together** are the destination guide: geometry and lower bounds, estimating equations, and likelihood constructions. Long technical verifications, historical source notes, and end-of-chapter exercises are deferred or compressed where explicitly labeled; complete conceptual proofs and the assumptions needed for the main route are retained.
...
The notation continues to follow van der Vaart, including a few deliberate reuses. The functional $\psi(P)$ is the statistical target. In the modern DML asides, $\psi(W;\theta,\eta)$ instead denotes a moment or score, following that literature; its displayed arguments distinguish it from the target functional. Likewise, $A_\eta$ and $A_{\theta,\eta}$ denote score operators in §§25.5 and 25.12. Section 25.9 therefore uses $J_{\theta,\tau}$ for the derivative normalization of an estimating equation, a notation local to these notes that avoids confusing that matrix with the book's score operator.

On a fixed path, the source writes $P_t$; these notes write $P_{t,g}$ when the score index must be explicit. In §25.6, $g$ is the tangent direction and $h$ is the scalar local displacement. Section 25.12 reuses $h$ as an index for a nuisance likelihood equation.
...
Section 25.2 supplies the functional-analytic language used throughout the chapter. Banach-space norms make infinite-dimensional differentiation and convergence precise, while Hilbert-space geometry supplies the projections, adjoints, and orthogonal decompositions used to identify efficient scores and influence functions.
...
::: {.callout-tip title="Handoff to §§25.3–25.5"}

Keep three operations in view: **project** onto a closed space, **move** an operator across an inner product with its adjoint, and **invert** the resulting information operator when that inversion is stable. Sections 25.3–25.5 use the first two to identify efficient influence functions; [§25.12](#sec-likelihood-equations) returns to the third when it linearizes joint likelihood equations in a Banach space.

:::

## 25.3 Tangent Spaces and Information

Section 25.3 turns regular paths into tangent directions, differentiates the target along those directions, and identifies the efficient influence function as the object that determines the sharp asymptotic covariance bound.
...
and plot the ordinary scalar curve $\varepsilon\mapsto T(P_\varepsilon)$. In their Figure 1, one panel shows the distribution changing from the truth $P$ toward an initial estimate $\widetilde P$, while the other records the target value along exactly the same path. Once a path has been chosen, a pathwise derivative is therefore an ordinary slope; the picture does not pretend to draw the whole infinite-dimensional model [@fisherKennedy2021, §3 and fig. 1].
...
- **Derivative algebra.** Treat already-known influence functions as building blocks and use product and chain rules. For example,
    ...
    [Section 25.7](#sec-efficiency-delta-method) gives the formal chain-rule argument and shows when a smooth transform preserves efficiency.
...
**Builds on.** The finite-dimensional reduction uses the Gaussian limit experiment from [Theorem 7.10](chapter-7.qmd#theorem-normal-limit-experiment), the [reduction in Theorem 8.3](chapter-8.qmd#theorem-limit-experiment-reduction), and the [local asymptotic minimax framework in Section 8.7](chapter-8.qmd#sec-local-asymptotic-minimax).
...
::: {.callout-tip title="Handoff from tangent geometry"}

Section 25.3 has now supplied both the **benchmark** and the **recognition rule**. [Theorem 25.20](#theorem-25-20) and [Theorem 25.21](#theorem-25-21) give the lower bounds, while [Lemma 25.23](#lemma-25-23) says an estimator reaches them exactly when its first-order summand is the efficient influence function. Sections [25.4](#efficient-score-functions) and [25.5](#score-and-information-operators) show how to calculate that function; Sections [25.6](#sec-testing)–[25.12](#sec-likelihood-equations) show what to do with it.

:::

## 25.4 Efficient Score Functions {#efficient-score-functions}

Section 25.4 specializes the tangent-space construction to a model with a finite-dimensional target and an infinite-dimensional nuisance parameter. Projecting the ordinary target score away from the nuisance-score space produces the efficient score; normalizing that residual by efficient information produces the efficient influence function.
...
::: {.callout-tip title="Handoff from projection to construction"}

[Section 25.8](#sec-efficient-score-equations) estimates and solves the efficient score, while [§25.9](#sec-general-estimating-equations) allows other valid residual weights and measures their efficiency loss.

:::

## 25.5 Score and Information Operators {#score-and-information-operators}

Sections 25.3–25.4 describe efficiency through tangent spaces and projections. Section 25.5 expresses the same geometry through score operators, a formulation that remains useful when:
...
Section 25.5.1 is the operator version of the projection argument in [§25.4](#efficient-score-functions). For the model $\{P_{\theta,\eta}:\theta\in\Theta,\eta\in H\}$, let $\mathbb H_\eta=\overline{\operatorname{lin}}\dot H_\eta$ denote the closed linear span of the nuisance directions. A joint direction $(a,b)$ perturbs the target by $a\in\mathbb R^k$ and the nuisance parameter by $b\in\mathbb H_\eta$. The nuisance score operator
...
Section 25.5.2 applies score operators when the observed data retain only part of the full-data information. The central fact is that the score operator becomes conditional expectation.
...
Section 25.5.3 specializes the information-loss framework to missing and coarsened data. The score spaces for the full-data law and the coarsening mechanism determine which observed-data influence functions are valid and which one is efficient.
...
::: {.callout-tip title="Handoff from operators to procedures"}

Sections 25.2–25.5 answer the chapter's identification question: form the tangent space, remove nuisance directions, and solve the resulting projection or adjoint equation. The examples have also introduced the three main application mechanisms—hazard-score projection, conditional-expectation operators, and MAR augmentation. The rest of the chapter holds the resulting efficient influence function fixed and asks how to use it for [testing](#sec-testing), [estimating equations](#sec-efficient-score-equations), and [likelihood inference](#sec-maximum-likelihood-estimators).

:::

## 25.6 Testing {#sec-testing}

Section 25.6 derives the largest attainable local power for detecting root-$n$ departures from a semiparametric null and shows that a test built from an efficient estimator attains that bound.
...
| Theorems 25.47–25.48 and Lemma 25.49 | Low | Recognize the Banach-space extension; proofs are not essential here |
...
![](images/generated/chapter-15-local-power.svg){fig-alt="At a fixed norm for the tangent displacement, three increasing local-power curves show that a direction aligned with the efficient influence function yields the largest power, while partially aligned directions yield less power."}
...
This is the same Gaussian calculation as @fifteenfig-ch15-local-power. Theorem 25.44 changes the object that supplies the alignment coefficient, not the shape of the power curve.
...
Take any subsequence along which the limsup in the theorem is attained at $(h_1,g_1)$. The compactness argument in [Theorem 15.1](chapter-15.qmd#theorem-15-1) and contiguity give a further subsequence whose power functions converge to the power function $\pi(h,a)$ of a test in this Gaussian experiment.
...
[Proposition 15.2](chapter-15.qmd#proposition-15-2) now gives, for $a^Tv>0$,
...
::: {.callout-important title="Section 25.7 in one sentence"}
...
**Earlier / new here.** The ordinary [delta method in Theorem 3.1](chapter-3.qmd#theorem-3-1) explains the finite-dimensional transformation step, while the [functional delta method in Theorem 20.8](chapter-20.qmd#theorem-20-8) and its [chain rule](chapter-20.qmd#theorem-20-9) supply the Hadamard-differentiable version for random elements. [Lemma 25.23](#lemma-25-23) adds the genuinely new conclusion: the transformed influence function is not merely valid, but efficient. Chapter 18 supplies [random elements](chapter-18.qmd#sec-random-elements), [tightness and Prokhorov's theorem](chapter-18.qmd#sec-tightness), and the process-level convergence needed to combine those ideas here.
...
For a path $t\mapsto P_t$ with score $g$, differentiability of $\psi$ and the Hadamard chain rule in [Theorem 20.9](chapter-20.qmd#theorem-20-9) give
...
The functional delta method, [Theorem 20.8](chapter-20.qmd#theorem-20-8), is now applicable tangentially to $\mathbb D_0$:
...
[Section 25.10.1](#sec-25-10-random-censoring) returns to the same estimator and derives it from the nonparametric likelihood, giving a complementary construction.
...
Fisher and Kennedy's Figure 1B draws (25.8-functional-one-step) as a tangent-line correction. The plug-in value $T(\widetilde P)$ is at one endpoint of the path, the tangent line is extrapolated toward the truth, and the remaining vertical gap is $R_2$. Equation (25.8-one-step) above starts instead from a finite-dimensional $\widetilde\theta_n$ and takes one Newton step on the efficient-score equation. Because $\widetilde I^{-1}\widetilde\ell$ is the efficient influence function, the two formulas have the same first-order architecture [@fisherKennedy2021, §3 and fig. 1; @kennedy2022, §§4.1–4.3].
...
::: {.callout-important title="Section 25.9 in one sentence"}
...
**Earlier / new here.** Section 25.4 found the uniquely efficient residual weight. Here the weight is allowed to vary, producing a family of valid estimators whose distance from that optimal weight measures the efficiency loss.
...
Van der Vaart states Example 25.67 with $\pi$ known or correctly specified and uses $c$ primarily to optimize variance. If instead both entries are treated as working nuisance functions, the same equation contains the algebra of [double robustness](glossary.qmd#glossary-double-robustness).
...
DML estimates $(\mu_0,\mu_1,e)$ on training folds and evaluates this score on held-out folds. In the [one-step decomposition](#sec-25-8-one-step), cross-fitting supplies the conditional independence used to control the estimated-function empirical-process term, together with $L_2$ convergence and moment bounds; the product identity (25.67-double-robust) supplies target-specific remainder control [@kennedy2022, §§4.2–4.3; @chernozhukovEtAl2018]. Causal interpretation additionally requires identification assumptions—most importantly consistency, conditional exchangeability, and overlap—which are not supplied by the missing-data geometry alone.
...
::: {.callout-important title="Section 25.10 in one sentence"}
...
Its maximizer is the product-limit, or Kaplan--Meier, estimator. [Lemma 20.14](chapter-20.qmd#lemma-20-14) develops the cumulative-hazard/product-integral map, and [Example 20.15](chapter-20.qmd#example-20-15) applies that map to Nelson--Aalen and Kaplan--Meier. If $t_1<\cdots<t_m$ are the distinct observed failure times, $d_j$ is the number of failures at $t_j$, and $r_j$ is the number at risk immediately before $t_j$, then
...
The gap matters most for infinite-dimensional nuisance estimators. As the examples in [Section 25.10](#sec-maximum-likelihood-estimators) show, a semiparametric MLE is often a step function or another boundary point. A formally correct efficient direction may destroy monotonicity, positivity, or smoothness when moved in both signs, so it cannot be used to differentiate the likelihood at the MLE.
...
::: {.callout-important title="Section 25.11 in one sentence"}
...
First establish consistency by the [Wald argument in Chapter 5](chapter-5.qmd#sec-5-2). The target parameter space is compact and the monotone cumulative hazards bounded by $M$ are compact in the weak topology. The distance from $(\widehat\theta,\widehat\Lambda)$ to the Kullback--Leibler maximizers therefore tends to zero. The observed model identifies $\theta_0$ and identifies $\Lambda_0(t)$ for every $\sigma<t<\tau$, so
...
Projection onto the nuisance-score space replaces $V$ by its conditional expectation given $W$; this is the conditional-expectation instance of [Theorem 11.1](chapter-11.qmd#theorem-11-1). Therefore
...
Read this within the standing setup of Section 25.8: the plug-in score also satisfies the $L_2(P_0)$ consistency requirement in (25.53). In the present model, given $\widehat\theta_n\to\theta_0$, it is enough that
...
[Section 25.8](#sec-efficient-score-equations) isolates the finite-dimensional target and solves one efficient score equation after estimating the nuisance parameter. [Section 25.11](#sec-approximately-least-favorable-submodels) shows how a likelihood maximizer can satisfy an equation that is first-order equivalent to that efficient score equation. Section 25.12 takes a different route: write the likelihood equations for the target and nuisance parameters **jointly**, treat them as one infinite-dimensional Z-estimation problem, and linearize the whole system.

::: {.callout-important title="Section 25.12 in one sentence"}
...
This route is closer to the ordinary multivariate proof for maximum likelihood, but it has a strong limitation: both $\widehat\theta-\theta_0$ and $\widehat\eta-\eta_0$ must be root-$n$ in the Banach-space norm used to formulate the equations. That fails in many natural parametrizations. For example, the current-status cumulative-hazard estimator in [Section 25.11.1](#sec-25-11-current-status-cox) converges at $n^{-1/3}$, which is why the target-specific approximately least-favorable argument is needed there.
...
Recall from [Section 25.5.1](#sec-semiparametric-score-operators) that the joint score operator is
...
These four blocks are the four pieces of the information operator $A_0^*A_0$ from [Section 25.5](#sec-semiparametric-score-operators), with a minus sign. This is the operator version of the parametric information identity
...
Section 25.12 closes the chapter by unifying likelihood, empirical-process, and information-operator arguments. Its method is powerful when the full nuisance parameter is itself regular at root-$n$ in a suitable norm. When that is too demanding, the target-specific orthogonal methods of [Sections 25.8](#sec-efficient-score-equations) and [25.11](#sec-approximately-least-favorable-submodels) are usually the more flexible route.
