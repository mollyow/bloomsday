...
For a parallel development of likelihood ratios, contiguity, and Le Cam's lemmas, see [@bailie2021, §§10.2.1–10.2.3 and §§11.1–11.2].

:::

::: {.callout-note title="Reading guide"}

Both source sections are on the direct route.

- **Core:** §6.1 and Lemma 6.2 establish exactly what a likelihood ratio represents when the two measures are not absolutely continuous.
- **Core:** §6.2, Lemma 6.4, and Theorem 6.6 characterize contiguity and give the abstract change-of-measure theorem.
- **Core mechanism:** Example 6.7 is the Gaussian form usually called Le Cam's third lemma. The normal-location calculation below makes its local-alternative shift explicit.
- **No substantive source section is omitted:** Example 6.5 is retained as a compact diagnostic for the lognormal likelihood-ratio limit.
...
**(1) $\Leftrightarrow$ (4).** If $T_n\overset{P_n}\to0$, apply contiguity to $A_n=\{\|T_n\|>\epsilon\}$ for each $\epsilon>0$. Conversely, given events $A_n$ with $P_n(A_n)\to0$, use $T_n=\mathbf1_{A_n}$.
...
$$
\left(\frac{dQ_n}{dP_n},\mathbf1_{A_n^c}\right)
...
\liminf_n Q_n(A_n^c)
\geq
\liminf_nE_{P_n}\left\{\mathbf1_{A_n^c}\frac{dQ_n}{dP_n}\right\}
\geq EV=1.
$$
...
Because every likelihood ratio is nonnegative, its weak limit satisfies $V\geq0$. Contiguity and [Lemma 6.4](#lemma-le-cam-first) give $EV=1$. Hence $B\mapsto E\{\mathbf1_B(X)V\}$ is a probability measure.
...
The cross-covariance $\tau=\operatorname{Cov}(X,W)$ determines the mean shift under the alternative. Exponential tilting changes the center but not the covariance.
...
::: {.callout-tip title="From contiguity to Chapter 25"}
