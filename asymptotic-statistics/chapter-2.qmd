...
For a systematic review of convergence and stochastic order, see [@jiang2022, §§2.1–2.7 and §3.4]. For a compact treatment centered on weak convergence, see [@dasgupta2008, §1.1].

:::

::: {.callout-note title="Reading guide"}

The route through van der Vaart's chapter is deliberately selective.

- **Core:** §2.1 through Lemma 2.8 supplies Portmanteau, continuous mapping, the Euclidean Prohorov–Helly compactness step, relations among convergence modes, and Slutsky's lemma.
- **Core:** §2.2 supplies the stochastic $o_P/O_P$ calculus and Lemma 2.12, which converts deterministic remainder bounds into stochastic ones.
- **Illustration:** Example 2.9 shows how weak convergence and a random normalizer combine in a $t$ statistic.
- **Deferred:** Example 2.10 and Lemma 2.11, characteristic functions, almost-sure representations, moment convergence, convergence-determining classes, the law of the iterated logarithm, Lindeberg--Feller theory, and total variation are not needed for the direct route to Chapter 25. [Chapter 18](chapter-18.qmd#sec-tightness) generalizes Prohorov's theorem and tightness to metric spaces.
...
Convergence in probability and almost-sure convergence require $X_n$ and $X$ to live on the same probability space. Weak convergence depends only on their laws.

::: {#twofig-ch2-convergence-map}

![](images/generated/chapter-2-convergence-map.svg){fig-alt="A diagram shows almost-sure convergence implying convergence in probability, which implies convergence in distribution. L1 convergence also implies convergence in probability. A separate note states that every subsequence has a further subsequence converging almost surely to X, and uniform integrability provides a route from convergence in probability to L1."}

The main implications among modes of stochastic convergence. The extra moment conditions shown in the diagram are not part of the route developed in this chapter.
...
Then $f_m$ is bounded and Lipschitz, $0\leq f_m\leq\mathbf1_G$, and $f_m\uparrow\mathbf1_G$. Hence, for every fixed $m$,
...
The theorem passes a known limit through a statistic. Its discontinuity caveat is substantive: if $X_n=1/n$, $X=0$, and $g(x)=\mathbf1\{x>0\}$, then $X_n\to0$ but $g(X_n)=1\not\to g(0)=0$.
...
Example 2.6 gives a quick sufficient condition for (2.4-tightness): if $\sup_nE\|X_n\|^p<\infty$ for some $p>0$, then Markov's inequality yields
...
Lemma 2.12 is the bridge from ordinary differentiability to stochastic expansions. It supplies the remainder step in the [delta method](chapter-3.qmd#theorem-3-1), and the same mechanism reappears in likelihood and estimating-equation expansions throughout Chapters 7, 19, and 25.
