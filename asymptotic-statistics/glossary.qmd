...
with the usual integrability qualification; outer probability is $P^*(A)=E^*\mathbf 1_A$. These envelopes let empirical-process statements remain meaningful when a supremum over an uncountable class is not known to be measurable. See [the formal definitions in §18.2](chapter-18.qmd#sec-outer-expectation) and their rate-theorem use in [Theorem 25.81](chapter-25.qmd#theorem-25-81).
...
### Convolution and local asymptotic minimax bounds {#glossary-efficiency-bounds}

The convolution bound says that the limit law of a regular estimator contains the efficient Gaussian component plus an independent noise component. The local asymptotic minimax bound expresses the same information restriction as a lower bound on the largest local risk under bowl-shaped loss. In Chapter 25 the covariance of the efficient Gaussian component is $P\widetilde\psi_P\widetilde\psi_P^T$. See the [Chapter 8 convolution theorem](chapter-8.qmd#sec-convolution-theorem), the [local asymptotic minimax theorem](chapter-8.qmd#sec-local-asymptotic-minimax), [Theorem 25.20](chapter-25.qmd#theorem-25-20), and [Theorem 25.21](chapter-25.qmd#theorem-25-21).
...
The orthogonal projection of $X\in L_2(P)$ onto a closed linear subspace $S$ is the unique element $\Pi_SX\in S$ such that $X-\Pi_SX$ is orthogonal to every element of $S$. The Pythagorean identity makes it the minimum-mean-square approximation. See Theorem 11.1 and its score-space use in [§25.4](chapter-25.qmd#efficient-score-functions).
...
Hadamard differentiability is the uniform directional differentiability used by the functional delta method. Fréchet differentiability is a norm-uniform linear approximation and is stronger. See §20.2 and Theorem 20.8, the [Banach-space delta-method guide in §25.7](chapter-25.qmd#sec-efficiency-delta-banach), and [the likelihood-equation linearization in §25.12](chapter-25.qmd#theorem-25-90).
...
Here the representative is chosen so that $\widehat P\psi_{\widehat P}^\dagger=0$. The plug-in or machine-learning fit supplies $\widehat P$; semiparametric theory supplies the correction, the efficiency benchmark, and the remainder conditions under which the result is asymptotically linear. With sample splitting or cross-fitting, the nuisance fit used in a summand is learned away from that observation, providing conditional independence; $L_2$ convergence, integrability, and target-specific remainder control are still required. See the finite-dimensional Newton prototype in Theorem 5.45 and [the influence-function route in §25.8](chapter-25.qmd#sec-25-8-one-step).
...
[Return to the route through the book](index.qmd#route-to-chapter-25).
