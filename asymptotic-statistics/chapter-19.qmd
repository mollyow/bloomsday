...
- **Chapter 25 payoff:** Donsker theory controls estimated scores in [Section 25.8](chapter-25.qmd#sec-efficient-score-equations), supplies nuisance-rate bounds in [Section 25.11](chapter-25.qmd#sec-approximately-least-favorable-submodels), and gives the Gaussian process used to linearize [likelihood equations in Section 25.12](chapter-25.qmd#sec-likelihood-equations).

:::

::: {.callout-note title="Companion readings"}

For complementary introductions, see [@sen2022, §§1.3--4 and ch. 11], [@kosorok2008, chs. 8--10 and §13.2], and [@jiang2022, ch. 7]. The standard comprehensive reference is @vanDerVaartWellner1996.

:::

### Route-only coverage

| Source material | Treatment here | Destination |
|---|---|---|
| §§19.1–19.2 | Core definitions, the complete bracketing route to Theorem 19.5, and selected entropy examples | Uniform laws and Donsker verification throughout Chapter 25 |
| §19.3 | Omitted except for its location in the source | Goodness-of-fit limits are not needed for the Chapter 25 route |
| §19.4 | Lemma 19.24, Example 19.25, and Theorem 19.26 with complete proofs | Estimated scores and infinite-dimensional estimating equations |
| §19.5 | Orientation only | Recognizing when a rescaled local class changes with $n$ |
| §19.6 | Bernstein's inequality, finite maxima, Lemma 19.34, and the bounded-class Lemma 19.36 | Bracketing proof of Theorem 19.5 and nuisance-rate calculations |

The source contains additional smoothness, VC, changing-class, and goodness-of-fit results. They remain useful as a reference, but the material retained here is the shortest route from empirical-process foundations to the score and likelihood equations in Chapter 25.
...
$1\{X_j\leq t_i\}$ and $1\{X_j<t_i\}$ gives
...
This is the functional, uniform analogue of the central limit theorem. Section 19.2 develops conditions under which the same conclusion holds for more general function classes.
:::
...
\leq
\frac1{a(\delta)}P^*F^2 1\{F>a(\delta)\sqrt n\}
\longrightarrow0.
$$
...
$$
\left[1_{(-\infty,t_{i-1}]},1_{(-\infty,t_i)}\right]
$$
...
VC classes provide another way to control complexity. A class has finite VC dimension if it cannot represent arbitrarily complicated patterns on finite sets. Finite VC dimension implies polynomial covering-number growth and, with suitable envelope moment conditions, Glivenko–Cantelli and Donsker properties.
...
::: {.callout-tip title="Example 19.25: mean absolute deviation"}
...
The theorem is the infinite-dimensional version of [Theorem 5.21](chapter-5.qmd#theorem-5-21). The index $h$ labels an entire system of equations; the parameter $\theta$ itself may also be infinite-dimensional.
...
This one picture links the empirical-CDF thread to Chapter 25: [Example 25.24](chapter-25.qmd#example-25-24) uses empirical averages as efficient estimators, while [Theorem 25.90](chapter-25.qmd#theorem-25-90) applies the same two-part Z-linearization to a full semiparametric likelihood system.
...
\begin{aligned}
A_{q-1}f
&=1\{\Delta_{q_0}f\leq\sqrt n\,a_{q_0},\ldots,
...
This is the quantity balanced against deterministic criterion curvature in [Theorem 5.55](chapter-5.qmd#theorem-5-55) and [Theorem 25.81](chapter-25.qmd#theorem-25-81). The [current-status Cox calculation](chapter-25.qmd#sec-25-11-current-status-cox) is a concrete instance.

:::

::: {.callout-note title="Lemma 19.37: stronger tail control"}
